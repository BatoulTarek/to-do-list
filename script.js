...
function addTodoItemToHTML(todoItemObj) {
  ...
  todoItemObj.completed
    ? (todoItem.className = 'todo-item-done')
    : (todoItem.className = 'todo-item');

  todoItem.setAttribute('data-id', todoItemObj.id);

  todoItem.innerHTML = `<div class="todo-text">
        <input type="checkbox" name="todo-checkbox" id="${
          'todo-checkbox-id-' + todoItemObj.id + ''
        }"  
        ${
          todoItemObj.completed ? 'checked' : ''
        }   class="todo-checkbox" onChange="toggleTodoStatusById(this)" "/> 
        <lable for="${
          'todo-checkbox-' + todoItemObj.id + 'id'
        }"  class="todo-title" data-id=${todoItemObj.id}>
...
todoListDiv.addEventListener('click', (event) => {
  ...
  if (event.target.classList.contains('todo-item')) {
    ...
    toggleTodoStatusByIDandTodoItem(id, todoItemElement);
    if (
      completedTodos.classList.contains('active') ||
      pendingTodos.classList.contains('active')
    ) {
      todoItemElement.remove();
    }
...
function toggleTodoStatusByIDandTodoItem(id, todoItemElement) {
  todoItemElement.classList.contains('todo-item-done')
    ? (todoItemElement.className = 'todo-item')
    : (todoItemElement.className = 'todo-item-done');
